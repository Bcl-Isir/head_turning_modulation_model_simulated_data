...
% === CONSTRUCTOR [BEG] === %
function obj = DynamicWeighting (htm, varargin)
	obj.htm = htm;
	obj.MFI = htm.MFI;
...
function computeAposterioriProbabilities (obj)
    ...
    if numel(obj.observed_categories) > 1
        ...
        denom = obj.htm.RIR.nb_objects;
    	if obj.source_env ~= 0
    		for iEnv = obj.source_env
	    		env = getEnvironment(obj, iEnv);
	    		av_cats = [av_cats; getObject(env, 'all', 'audiovisual_category')];
	    		denom = denom + numel(env.objects);
	    	end
    	end
        classes = unique(av_cats);
        for iCat = classes'
            if iCat ~= 0 && ~isempty(iCat) && isPerformant(obj, iCat)
                ...
                obj.MFI.observed_categories{iCat}.proba = obj.MFI.observed_categories{iCat}.cpt/denom;
            end
        end
...
function computeCongruence (obj)
	...
	for iCat = obj.classes'
		if iCat ~= 0
			if isPerformant(obj, iCat)
				if obj.MFI.observed_categories{iCat}.proba <= 1/obj.nb_classes
...
function computeWeights (obj)
	...
	for iObj = 1:obj.htm.RIR.nb_objects
		...
		if obj_cat ~= 0
			...
			% --- Compute weights thanks to weighting functions
				...
				if obj.MFI.observed_categories{obj_cat}.congruence == -1
					increaseObjectWeight(obj.htm, iObj);
				% --- Congruent
				elseif obj.MFI.observed_categories{obj_cat}.congruence == 1
